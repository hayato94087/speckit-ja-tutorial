...
validate_environment() {
    ...
    if [[ ! -f "$NEW_PLAN" ]]; then
        ...
        log_info "対応するspecディレクトリを持つ機能で作業していることを確認してください"
        if [[ "$HAS_GIT" != "true" ]]; then
            log_info "使用方法: export SPECIFY_FEATURE=your-feature-name またはまず新しい機能を作成"
        fi
        exit 1
    fi
...
parse_plan_data() {
    ...
    if [[ ! -r "$plan_file" ]]; then
        log_error "計画ファイルを読み取れません: $plan_file"
        return 1
    fi
    
    log_info "計画データを解析中: $plan_file"
    
    NEW_LANG=$(extract_plan_field "言語/バージョン" "$plan_file")
    NEW_FRAMEWORK=$(extract_plan_field "主要な依存関係" "$plan_file")
...
update_existing_agent_file() {
    ...
    if grep -q "^## 使用技術" "$target_file" 2>/dev/null; then
...
update_agent_file() {
    ...
    if [[ -z "$target_file" ]] || [[ -z "$agent_name" ]]; then
        log_error "update_agent_fileはtarget_fileとagent_nameパラメータが必要です"
        return 1
    fi
    ...
    if [[ ! -f "$target_file" ]]; then
        ...
        if create_new_agent_file "$target_file" "$temp_file" "$project_name" "$current_date"; then
            if mv "$temp_file" "$target_file"; then
                ...
                log_error "一時ファイルの移動に失敗しました: $target_file"
                rm -f "$temp_file"
                return 1
            fi
        else
...
update_specific_agent() {
    ...
    case "$agent_type" in
        ...
        *)
            ...
            log_error "有効な値: claude|gemini|copilot|cursor-agent|qwen|opencode|codex|windsurf|kilocode|auggie|roo|amp|shai|q|bob|qoder"
            exit 1
            ;;
    esac
}
...
print_summary() {
    ...
    log_info "使用方法: $0 [claude|gemini|copilot|cursor-agent|qwen|opencode|codex|windsurf|kilocode|auggie|codebuddy|shai|q|bob|qoder]"
}
...
main() {
    ...
    log_info "=== 機能 $CURRENT_BRANCH のエージェントコンテキストファイルを更新中 ==="
    
    # Parse the plan file to extract project information
    if ! parse_plan_data "$NEW_PLAN"; then
        log_error "計画データの解析に失敗しました"
        exit 1
    fi
    ...
    if [[ -z "$AGENT_TYPE" ]]; then
        ...
        log_info "エージェントが指定されていません。すべての既存エージェントファイルを更新します..."
        if ! update_all_existing_agents; then
            success=false
        fi
    else
        # Specific agent provided - update only that agent
        log_info "指定されたエージェントを更新中: $AGENT_TYPE"
    ...
    if [[ "$success" == true ]]; then
        log_success "エージェントコンテキストの更新が正常に完了しました"
        exit 0
    else
        log_error "エージェントコンテキストの更新がエラーで完了しました"
        exit 1
    fi
}
