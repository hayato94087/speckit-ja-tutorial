...
- **NFR-001**: 型チェック、リント、フォーマットチェックは1分以内に完了しなければならない（1,000ファイル未満の場合）
- **NFR-002**: Unitテストは2分以内に完了しなければならない（100テストケース未満の場合）
- **NFR-003**: E2Eテストは5分以内に完了しなければならない（主要導線シナリオのみの場合）
- **NFR-004**: コマンド出力は色分けされ、エラー/警告/成功が視覚的に区別できなければならない
- **NFR-005**: 並列実行可能な検証は並列で実行され、全体の実行時間を短縮しなければならない
...
5. **ドキュメント完備**: 各品質ゲートの実行方法と期待結果がREADMEに記載されている
