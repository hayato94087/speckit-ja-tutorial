...
- [x] **追加→リロード→復元**: タスクを追加し、ブラウザをリロードして復元される
- [x] **完了/未完了切替**: チェックボックスで切替、視覚的に区別される
- [x] **編集（保存）**: タスク名を編集し、Enter で保存される
- [x] **編集（キャンセル）**: 編集中に Escape で元に戻る
- [x] **削除**: タスクを削除し、一覧から消える
- [x] **削除の誤操作対策**: 削除後に Undo 通知、または確認UIがある
...
- [x] 「未完了」で未完了タスクのみ表示
- [x] 「完了済み」で完了タスクのみ表示
- [x] フィルタ切替後もリロードで状態維持（任意）

### 8.3 データ堅牢性

- [x] **破損データ**: localStorage に不正 JSON を直接設定し、アプリを開いてもクラッシュしない
- [x] **旧形式データ**: 旧形式（idがnumber等）を設定し、変換または警告が表示される
- [x] **容量警告**: ストレージ上限に近い状態で警告が表示される（手動確認困難なら省略可）

### 8.4 アクセシビリティ

- [x] **Tab キーで全要素にフォーカス移動**: 入力→追加→各タスク→フィルタの順
- [x] **フォーカス可視化**: フォーカス時に視覚的なリングが表示される
- [x] **Enter/Space でアクション実行**: ボタン・チェックボックスが操作できる
- [x] **Escape でキャンセル**: 編集モード終了
- [x] **スクリーンリーダーでラベル読み上げ**: VoiceOver/NVDAで確認（可能な範囲）

### 8.5 レスポンシブ

- [x] **320px幅**: DevToolsでモバイル幅にして全機能が操作可能
- [x] **タップ領域**: ボタン・チェックボックスが指でタップしやすいサイズ

### 8.6 パフォーマンス

- [x] **1,000件タスク**: コンソールからタスクを1,000件追加し、操作が0.3秒以内
