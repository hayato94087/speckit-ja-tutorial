...
- [2. ユーザーストーリー（必須）](#-2-ユーザーストーリー必須)
  - [2.1 US1: タスクの素早い追加](#21-us1-タスクの素早い追加-p1)
  - [2.2 US2: タスクの完了管理](#22-us2-タスクの完了管理-p1)
  - [2.3 US3: タスクの編集と削除](#23-us3-タスクの編集と削除-p1)
  - [2.4 US4: タスク一覧のフィルタリング](#24-us4-タスク一覧のフィルタリング-p2)
  - [2.5 US5: データの永続化と復旧](#25-us5-データの永続化と復旧-p1)
  - [2.6 エッジケース](#26-エッジケース)
- [3. 要件（必須）](#-3-要件必須)
  - [3.1 機能要件](#31-機能要件)
  - [3.2 非機能要件](#32-非機能要件)
  - [3.3 主要エンティティ](#33-主要エンティティ)
- [4. 成功基準（必須）](#-4-成功基準必須)
...
4. 旧形式のデータを読み込む（可能な場合）
   - **Given**: 以前のバージョンの形式で保存されたデータがある
   - **When**: アプリを開く
   - **Then**: データが新形式に変換されて読み込まれる
...
- **非常に長いタスク名**: 長いタスク名は切り詰めて表示し、全文はツールチップや編集時に確認できる
- **タスクが0件**: フィルタ結果が0件の場合、適切なメッセージを表示する
- **大量のタスク**: 数百件のタスクがあっても、スクロールやパフォーマンスが劣化しない
- **ストレージ容量不足**: ローカルストレージの容量上限に達した場合、エラーを表示し、データ損失を防ぐ
- **同時編集の競合**: 複数のタブで同じ ToDo を開いている場合の挙動（最後の保存が優先される）
...
- **タスク（Task）**: ToDo リストの1項目を表す。タスク名、完了状態、作成日時、（任意で）更新日時を持つ
